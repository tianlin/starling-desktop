...
try:
    ...
    with sync_playwright() as pw:
        ...
        page.get_by_role('tab', name='评论', exact=True).click()
        cards = page.locator('.comment-list > .comment')
        ...
        expect(cards).to_have_count(2)
        expect(cards.first).to_contain_text('合成评论：这一段')
        expect(page.locator('.comment-published')).to_contain_text('合成浏览器发表')
        ...
        draft.fill('排序切换保留草稿')
        page.get_by_role('tab', name='最新', exact=True).click()
        expect(cards.first).to_contain_text('合成浏览器发表')
        expect(draft).to_have_value('排序切换保留草稿')
        expect(page.locator('.comment-published .comment')).to_have_count(0)
        page.get_by_role('button', name='加载更多评论', exact=True).click()
        expect(cards).to_have_count(4)
        assert any(r['action']=='comments.list' and r['payload'].get('order')=='latest' for r in reads)
        page.get_by_role('tab', name='热门', exact=True).click()
        expect(cards.first).to_contain_text('合成评论：这一段')
        page.get_by_role('tab', name='最新', exact=True).click()
        expect(cards).to_have_count(4)
        latest_tab = page.get_by_role('tab', name='最新', exact=True)
        latest_tab.focus()
        latest_tab.press('ArrowLeft')
        expect(page.get_by_role('tab', name='热门', exact=True)).to_be_focused()
        page.get_by_role('tab', name='热门', exact=True).press('End')
        expect(latest_tab).to_be_focused()
        expect(latest_tab).to_have_attribute('aria-selected', 'true')
        page.locator('.main').evaluate('(e)=>e.scrollTop=120')
        latest_scroll = page.locator('.main').evaluate('(e)=>e.scrollTop')
        page.get_by_role('tab', name='热门', exact=True).evaluate('(e)=>e.click()')
        page.locator('.main').evaluate('(e)=>e.scrollTop=200')
        latest_tab.evaluate('(e)=>e.click()')
        assert abs(page.locator('.main').evaluate('(e)=>e.scrollTop') - latest_scroll) < 2
        results.append('PASS: server hot/latest tabs keep independent caches, drafts, scroll, keyboard and publication feedback')
        draft.fill('跨页面草稿')
        page.get_by_role('tab', name='节目说明', exact=True).click()
        page.get_by_role('tab', name='评论', exact=True).click()
        ...
        # Every publication below uses the synthetic provider, never a real episode.
        page.get_by_role('tab', name='热门', exact=True).click()
        primary = page.locator('.comment-list > .comment[data-comment-id="64db2d493fa4090b744c5100"]')
        toggle = primary.locator('.comment-replies-toggle')
        if toggle.get_attribute('aria-expanded') != 'true': toggle.click()
        nested = primary.locator('.comment-replies .comment[data-comment-id="64db2d493fa4090b744c5103"]')
        expect(nested).to_be_visible()
        draft.fill('独立一级评论草稿')
        primary.locator(':scope > .comment-reply-action').click()
        expect(draft).to_be_focused()
        expect(page.locator('.comment-reply-target')).to_contain_text('合成听众')
        draft.fill('回复一级评论的草稿')
        nested.locator('.comment-reply-action').click()
        expect(draft).to_have_value('')
        draft.fill('回复楼中楼的草稿')
        page.get_by_role('button', name='取消回复', exact=True).click()
        expect(draft).to_have_value('独立一级评论草稿')
        primary.locator(':scope > .comment-reply-action').click()
        expect(draft).to_have_value('回复一级评论的草稿')
        page.get_by_role('tab', name='最新', exact=True).click()
        expect(draft).to_have_value('回复一级评论的草稿')
        page.get_by_role('tab', name='热门', exact=True).click()
        page.get_by_role('button', name='发送回复', exact=True).click()
        expect(draft).to_have_value('')
        expect(primary.locator('.comment-replies .comment').filter(has_text='回复一级评论的草稿')).to_have_count(1)
        assert cards.filter(has_text='回复一级评论的草稿').count() == 1  # ancestor contains its own thread
        assert not any('回复一级评论的草稿' == text for text in cards.locator(':scope > .comment-text').all_text_contents())
        results.append('PASS: per-target drafts, cancel, sort switching, primary reply and thread readback')
        nested.locator('.comment-reply-action').click()
        expect(draft).to_have_value('回复楼中楼的草稿')
        before_writes = len([r for r in reads if r['action']=='comments.create'])
        page.route('**/api', hold_created)
        held.clear()
        page.get_by_role('button', name='发送回复', exact=True).click()
        for _ in range(100):
            ...
            page.wait_for_timeout(20)
        assert len(held)==1
        expect(page.get_by_role('button', name='取消回复', exact=True)).to_be_disabled()
        expect(primary.locator(':scope > .comment-reply-action')).to_be_disabled()
        held[0][0].fulfill(response=held[0][1])
        page.unroute('**/api', hold_created)
        expect(draft).to_have_value('')
        confirmed_reply = primary.locator('.comment-replies .comment').filter(has_text='回复楼中楼的草稿')
        expect(confirmed_reply).to_have_count(1)
        expect(confirmed_reply).to_contain_text('回复 @合成听众')
        latest_write = [r['payload'] for r in reads if r['action']=='comments.create'][-1]
        assert latest_write['replyToCommentId']=='64db2d493fa4090b744c5103'
        assert latest_write['primaryCommentId']=='64db2d493fa4090b744c5100'
        assert len([r for r in reads if r['action']=='comments.create']) == before_writes+1
        results.append('PASS: nested reply carries direct target, flat quoted relation and one locked write')
        def reject_reply(route):
            if route.request.post_data_json['action']=='comments.create':
                route.fulfill(json={'ok':False,'error':{'code':'FORBIDDEN','message':'合成目标禁止回复'}})
            else: route.continue_()
        page.route('**/api', reject_reply)
        draft.fill('失败保留回复草稿')
        page.get_by_role('button', name='发送回复', exact=True).click()
        expect(page.locator('.comment-send-error')).to_contain_text('合成目标禁止回复')
        expect(draft).to_have_value('失败保留回复草稿')
        expect(page.locator('.comment-reply-target')).to_contain_text('合成听众')
        page.unroute('**/api', reject_reply)
        page.get_by_role('button', name='取消回复', exact=True).click()
        primary.locator(':scope > .comment-reply-action').click()
        draft.fill('合成回复已发送但结果丢失')
        page.route('**/api', lost_created_response)
        page.get_by_role('button', name='发送回复', exact=True).click()
        expect(page.locator('.comment-send-error')).to_contain_text('合成发表响应丢失')
        page.unroute('**/api', lost_created_response)
        before_writes = len([r for r in reads if r['action']=='comments.create'])
        page.get_by_role('button', name='刷新回复', exact=True).click()
        expect(primary.locator('.comment-replies .comment').filter(has_text='合成回复已发送但结果丢失')).to_have_count(1)
        expect(draft).to_have_value('合成回复已发送但结果丢失')
        page.get_by_role('button', name='已确认发表，清除草稿', exact=True).click()
        expect(draft).to_have_value('')
        assert len([r for r in reads if r['action']=='comments.create'])==before_writes
        assert page.locator('#audio').evaluate('(a)=>a===window.__commentAudio && !a.paused')
        results.append('PASS: denied and uncertain replies retain target/draft; thread refresh never resends, audio continuous')
        expect(primary.locator('.comment-replies-toggle')).to_have_text('收起回复（4）')
        draft.fill('退出前的合成回复草稿')
        ...
        expect(page.locator('.comments-panel')).to_have_count(0)
        held[0][0].fulfill(response=held[0][1])
        page.unroute('**/api', hold_created)
        ...
        page.get_by_role('tab', name='评论', exact=True).click()
        expect(draft).to_have_value('')
        expect(page.locator('.comment-reply-target')).to_have_count(0)
        expect(page.get_by_role('tab', name='热门', exact=True)).to_have_attribute('aria-selected', 'true')
        results.append('PASS: late reply after logout cannot restore old target, draft or selected order in new epoch')
