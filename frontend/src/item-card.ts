...
export function renderItemCard(it: Item, actions: ItemCardActions, local?: 'queue' | 'bookmarks', updates = false): HTMLElement {
        ...
        if (updates && it.kind === 'episode') controls.append(button('＋ 稍后听', () => actions.queue('append', it).catch(e => actions.notice(e)), 'button small'));
        ...
        const act = (text: string, fn: () => Promise<unknown>) => menu.append(button(text, () => { more.open = false; void fn().catch(e => actions.notice(e)); }, 'menu-item'));
        if (it.kind === 'episode') {
