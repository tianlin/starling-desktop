...
export function renderNotes(html: string, onTime: (n: number) => void, onLink: (url: string) => void): HTMLElement {
    const target = el('div', 'shownotes');
    const template = document.createElement('template');
    template.innerHTML = html.slice(0, 200000);
    let count = 0;
    function text(s: string, parent: Node) {
    ...
    function visit(n: Node, parent: Node, depth: number) {
