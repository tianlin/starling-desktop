...
export function showLink(app: Application) {
    if (!app.boot)
        return;
    ...
    body.append(form);
    form.addEventListener('submit', e => { e.preventDefault(); go.disabled = true; const epoch = app.boot.session.epoch; void call<Item>('openLink', { epoch, text: field.input.value }).then(it => { if (epoch === app.boot.session.epoch && app.modal.open) {
        app.modal.close();
        app.showDetail(it);
    } }).catch(e => { error.textContent = describeError(e); }).finally(() => { go.disabled = false; }); });
    field.input.focus();
}
export function showAccount(app: Application) {
    if (!app.boot)
        return;
    ...
    const enable = async () => { if (!check.checked)
        throw Error('请先阅读并确认账号接入风险。'); app.boot.settings.experimentalAccount = true; await app.saveSettings(); };
    const send = button('发送验证码', async () => {
        ...
        try {
            ...
            timer = setInterval(() => { send.textContent = `${--left}s 后可重发`; if (left <= 0) {
                clearInterval(timer);
                send.disabled = false;
...
export function showSettings(app: Application) {
    ...
    const section = (title: string, detail: string, ...controls: HTMLElement[]) => { const row = el('section', 'setting-row'); const copy = el('div'); copy.append(el('h3', '', title), el('p', 'muted', detail)); row.append(copy, ...controls); panel.append(row); };
    ...
    enabled.addEventListener('change', () => { if (enabled.checked) {
