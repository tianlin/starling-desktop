...
const titles: Record<string, string> = { home: '继续收听', subscriptions: '我的订阅', favorites: '收藏单集', bookmarks: '本地书签', queue: '稍后听', settings: '设置' };
const navIcons: Record<string, string> = { home: '◷', subscriptions: '▤', favorites: '♡', bookmarks: '▱', queue: '☷', settings: '⚙' };
export class Application {
    boot!: Bootstrap;
    desktop: DesktopInfo = { tray: false, mediaKey: false, demo: false };
    player: Player;
    readonly page = document.querySelector<HTMLElement>('#page')!;
    readonly modal = document.querySelector<HTMLDialogElement>('#modal')!;
    private route = 'home';
    private routeGeneration = 0;
    private list: Library | null = null;
    private listKind = '';
    private podcastID = '';
    private filter = '';
    ...
    constructor() {
        ...
        this.player.onEnded = () => { const next = this.boot.queue[0]; if (next)
            void this.player.play(next); };
    }
    async start() {
        this.wire();
        try {
            this.boot = await call<Bootstrap>('bootstrap');
            this.desktop = await call<DesktopInfo>('desktop.info').catch(() => this.desktop);
    ...
    private wire() {
        ...
        document.querySelector<HTMLInputElement>('#volume')!.addEventListener('input', e => {
            if (!this.boot)
                return;
            this.boot.settings.volume = Number((e.target as HTMLInputElement).value);
        ...
        document.querySelector<HTMLSelectElement>('#rate')!.addEventListener('change', e => {
            if (!this.boot)
                return;
            this.boot.settings.rate = Number((e.target as HTMLSelectElement).value);
        ...
        document.addEventListener('keydown', e => { if (e.code === 'Space' && !e.repeat && canUseSpace(e.target as HTMLElement)) {
            e.preventDefault();
    ...
    private nativeEvents() {
        ...
        if ('mediaSession' in navigator) {
            ...
            if (!this.desktop.mediaKey) {
                set('play', () => { if (this.player.state !== 'playing')
                    void this.player.toggle(); });
    ...
    async reload() { this.boot = await call<Bootstrap>('bootstrap'); this.applySettings(); this.drawAccount(); }
    drawAccount() {
        if (!this.boot)
            return;
    ...
    notice(e: unknown) { const n = document.querySelector<HTMLElement>('#notification')!; n.replaceChildren(el('span', '', typeof e === 'string' ? e : describeError(e)), button('关闭', () => { n.hidden = true; }, 'text-button')); n.hidden = false; }
    async logout() { this.player.pause(); await this.player.persist(); const epoch = this.boot.session.epoch; let failure: unknown; try {
        ...
        this.notice(failure); }
    async queue(op: string, it?: Item) { const epoch = this.boot.session.epoch; const data = await call<Item[]>('queue', { epoch, op, item: it ?? {} }); if (epoch !== this.boot.session.epoch)
        return; this.boot.queue = data; this.drawPlayer(); if (this.route === 'queue')
        this.drawLocal('queue'); }
    async bookmark(op: string, it: Item) { const epoch = this.boot.session.epoch; const data = await call<Item[]>('bookmarks', { epoch, op, item: it }); if (epoch !== this.boot.session.epoch)
        return; this.boot.bookmarks = data; if (this.route === 'bookmarks')
        this.drawLocal('bookmarks');
    else
        this.notice(op === 'remove' ? '已移除本地书签。' : '已保存本地书签，仅此设备可见。'); }
    async external(it: Item) { try {
    ...
    heading(title: string, sub: string, actions?: HTMLElement): HTMLElement { const h = el('div', 'page-heading'); const text = el('div'); text.append(el('div', 'eyebrow', 'YOUR LISTENING SPACE'), el('h1', '', title), el('p', 'muted', sub)); h.append(text); if (actions)
        h.append(actions); return h; }
    private async drawHome() {
    ...
    private drawLocal(kind: 'queue' | 'bookmarks') {
        const items = kind === 'queue' ? this.boot.queue : this.boot.bookmarks;
        const actions = el('div', 'actions');
        if (kind === 'queue' && items.length)
            actions.append(button('清空待播', () => { if (confirm('清空稍后听队列？当前播放不会停止。'))
    ...
    private itemCard(it: Item, local?: 'queue' | 'bookmarks'): HTMLElement {
        ...
        const act = (text: string, fn: () => Promise<unknown>) => menu.append(button(text, () => { more.open = false; void fn().catch(e => this.notice(e)); }, 'menu-item'));
        if (it.kind === 'episode') {
    ...
    private async loadLibrary(mode: string): Promise<boolean> {
        ...
        try {
            ...
            if (generation !== this.routeGeneration || op !== this.libraryGeneration)
                return false;
            const view = await call<Library>('library', { epoch, kind: this.listKind, podcastId: this.podcastID, mode });
    ...
    async details(it: Item) {
        ...
        try {
            const detail = await call<Item>('detail', { epoch, kind: it.kind, id: it.id });
    ...
    drawPlayer() {
        ...
        const old = document.querySelector<HTMLElement>('#player-art')!;
