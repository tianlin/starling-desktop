...
export function sortUpdates(items) {
    const time = (it) => { const n = Date.parse(it.published ?? ''); return Number.isFinite(n) ? n : -Infinity; };
    return [...items].sort((a, b) => (time(b) - time(a) || (a.id < b.id ? -1 : a.id > b.id ? 1 : 0)));
}
export function dateGroup(published, now = new Date()) {
    const d = new Date(published ?? '');
    if (!Number.isFinite(d.getTime()))
        return '日期未知';
    const key = (v) => `${v.getFullYear()}-${String(v.getMonth() + 1).padStart(2, '0')}-${String(v.getDate()).padStart(2, '0')}`;
...
export class UpdatesController {
    api;
    now;
    state = { view: null, loading: false, error: '', lastSuccess: 0, displayPage: 0, scrollTop: 0 };
    onChange = () => { };
    onUnauthorized = () => { };
    session;
    active = false;
    ...
    async enter() {
        ...
        if (!this.refreshed || this.now() - this.state.lastSuccess >= 600000)
    ...
    async load(mode) {
        if (!this.active || this.session?.state !== 'connected' || this.state.loading || (mode === 'more' && !this.state.view?.cursor))
            return;
        ...
        try {
            ...
            if (!current())
                return;
            const view = await this.api('library', { epoch, kind: 'updates', podcastId: '', mode });
            if (!current() || view.epoch !== epoch)
                return;
            if (mode === 'cached') {
                this.cacheRead = true;
                ...
                if (!this.state.view?.items.length && view.items.length)
...
export function renderUpdates(c, card) {
    const root = el('div', 'updates-view'), state = c.state, view = state.view;
    const tools = el('div', 'actions');
    const refresh = button(state.loading ? '加载中…' : '刷新', () => c.load('refresh'));
    refresh.disabled = state.loading;
    tools.append(refresh);
    if (state.loading)
        tools.append(button('取消加载', () => c.cancel()));
    root.append(tools);
    ...
    root.append(status);
    if (state.error) {
        const error = el('div', 'inline-error', state.error);
    ...
    for (const it of items.slice(offset, offset + 60)) {
        const next = dateGroup(it.published);
        if (next !== group) {
            list.append(el('h2', 'updates-date', next));
            group = next;
        }
        list.append(card(it));
    }
    if (!items.length && !state.loading)
        list.append(empty(view?.complete && !state.error ? '暂无订阅更新' : '尚未取得更新', view?.complete && !state.error ? '已返回完整的空列表。' : '请刷新重试；加载失败或分页不明不表示没有更新。'));
    root.append(list);
    if (items.length > 60) {
        const paging = el('div', 'pagination');
        const back = button('上一屏', () => { state.displayPage--; c.onChange(); });
        back.disabled = state.displayPage === 0;
        const next = button('下一屏', () => { state.displayPage++; c.onChange(); });
    ...
    if (view?.cursor) {
        const more = button('加载下一页', () => c.load('more'), 'button load-more');
