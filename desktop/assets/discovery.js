...
const initial = () => ({ query: '', resultQuery: '', kind: 'podcast', page: null, loading: false, error: '', history: [], historyError: '', historyLoading: false, suggestions: [], suggestionsError: '', suggestionsLoading: false, scrollTop: 0, creator: null, creatorLoading: false, creatorError: '', creatorScroll: 0, showCreator: false });
...
export class DiscoveryController {
    ...
    onChange = () => { };
    onLogin = () => { };
    onLink = async (_query, _current) => { };
    onUnauthorized = () => { };
    onSubscriptions = (_states) => { };
    session;
    active = false;
    generation = 0;
    token = 0;
    ...
    pendingReady = false;
    cursors = new Set();
    creatorCursors = new Set();
    constructor(api) {
        this.api = api;
    }
    setSession(session, seed = 0) {
    ...
    async history(mode, query = '') {
        ...
        this.onChange();
        try {
            const r = await this.api('discovery.history', { epoch, mode, query });
    ...
    async suggestions() {
        ...
        this.onChange();
        try {
            const r = await this.api('discovery.suggestions', { epoch });
    ...
    async search(more = false) {
        ...
        try {
            const result = await this.api('discovery.search', { epoch, generation, query, kind, cursor });
            if (!current())
                return;
    ...
    async openCreator(creator) { this.invalidateIntent(); this.state.creator = { creator, items: [], subscriptions: {}, cursor: '', complete: false }; this.state.showCreator = true; this.state.creatorScroll = 0; this.creatorCursors.clear(); await this.loadCreator(); }
    async loadCreator(more = false) {
        ...
        try {
            const result = await this.api('discovery.creator', { epoch, id, cursor });
            if (!current())
                return;
...
export function renderDiscovery(c, card) {
    ...
    const kinds = ['podcast', 'episode', 'user'];
    ...
    root.append(tabs);
    if (s.page && s.resultQuery && s.resultQuery !== s.query)
        root.append(el('p', 'muted', `当前显示“${s.resultQuery}”的搜索结果`));
    if (s.loading)
...
function creatorArt(c) { return cover({ id: c.id, title: c.nickname, image: c.avatar, kind: 'podcast', sourceUrl: '', restricted: false }, true); }
