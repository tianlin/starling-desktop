...
const freshPage = () => ({ items: [], cursor: '', complete: false, loaded: false, loading: false, error: '', cursors: new Set(), revision: 0, stale: false, scroll: 0 });
const freshComposer = () => ({ draft: '', uncertain: false, sendError: '', sent: false, replyTarget: undefined });
const freshThread = () => ({ ...freshPage(), confirmed: new Map() });
function freshEpisode() {
    const episode = { order: 'hot', pages: { hot: freshPage(), latest: freshPage() }, threads: new Map(), expanded: new Set(), composers: new Map([['', freshComposer()]]), targetKey: '', sending: false, confirmed: new Map() };
    // Consumers see the selected page; requests retain the concrete page they started with.
    for (const key of Object.keys(freshPage())) {
    ...
    for (const key of Object.keys(freshComposer())) {
        Object.defineProperty(episode, key, {
...
export class CommentsController {
    ...
    setSession(session) {
        ...
        if (key === this.sessionKey) {
            if (changed) {
                for (const episode of this.cache.values())
                    for (const page of [...Object.values(episode.pages), ...episode.threads.values()]) {
                        page.revision++;
                        page.loading = false;
                    }
                this.onChange?.();
        ...
        if (this.episodeId)
            this.cache.set(this.episodeId, this.state);
        this.onChange?.();
    }
    open(episodeId) {
        this.leave();
    ...
    async selectOrder(order) {
        ...
        this.onChange?.();
        if (!this.state.loaded || this.state.stale)
            await this.load();
    }
    rememberScroll(scroll, order = this.state.order) { this.state.pages[order].scroll = Math.max(0, scroll); }
    selectReply(commentId, primaryCommentId) {
        if (this.state.sending)
            return false;
        let source, primary = primaryCommentId ?? '';
        for (const page of Object.values(this.state.pages)) {
            const root = page.items.find(item => item.id === commentId);
            if (root && (!root.primaryCommentId || root.primaryCommentId === root.id)) {
                source = root;
                primary = root.id;
                break;
            }
        ...
        if (!source && primary)
            source = this.state.threads.get(primary)?.items.find(item => item.id === commentId && item.primaryCommentId === primary);
        let composer = this.state.composers.get(commentId);
        if (!composer) {
            composer = freshComposer();
            this.state.composers.set(commentId, composer);
        }
        if (!composer.replyTarget?.rejected)
            composer.replyTarget = { id: commentId, primaryCommentId: primary, nickname: source?.author.nickname || '这位听友', summary: source?.text.slice(0, 160) || '原评论暂不可确认', valid: !!source && !!primary };
        this.state.targetKey = commentId;
        if (!composer.replyTarget?.valid)
            composer.sendError = '无法确认回复关系，请刷新评论或所属回复后重新选择。';
        this.onChange?.();
        return true;
    }
    cancelReply() {
    ...
    async submit(explicitRetry = false) {
        ...
        if (replyTarget && !replyTarget.valid) {
            composer.sendError = '无法确认回复关系，请刷新评论或所属回复后重新选择。';
            this.onChange?.();
            return;
        }
        ...
        this.onChange?.();
        try {
            const result = await this.call('comments.create', { epoch, episodeId, text, requestId: crypto.randomUUID(), ...(replyTarget ? { replyToCommentId: replyTarget.id, primaryCommentId: replyTarget.primaryCommentId } : {}) });
            if (key !== this.sessionKey)
                return;
            if (!isComment(result?.comment) || result.comment.author.id !== this.authorId)
                throw new Error('发表响应无法确认。');
            if (replyTarget && (result.comment.primaryCommentId !== replyTarget.primaryCommentId || result.comment.replyTo?.id !== replyTarget.id))
                throw new Error('回复关系无法确认。');
            if (!replyTarget && (result.comment.replyTo || (result.comment.primaryCommentId && result.comment.primaryCommentId !== result.comment.id)))
                throw new Error('发表响应无法确认。');
            composer.draft = '';
            composer.uncertain = false;
            composer.sent = true;
            if (replyTarget) {
                ...
                thread.confirmed.set(result.comment.id, result.comment);
                thread.stale = true;
                ...
                if (this.state === target && this.episodeId === episodeId)
                    void this.fetchPage(target, target.pages[target.order], episodeId, target.order, undefined, true);
            }
        }
        catch (e) {
            if (key !== this.sessionKey)
            ...
            if (replyTarget && e instanceof APIError && e.code === 'COMMENT_TARGET_INVALID') {
                replyTarget.valid = false;
                replyTarget.rejected = true;
            }
            if (e instanceof APIError && e.code === 'UNAUTHORIZED')
                composer.sendError += ' 请在账号与连接中重新连接；草稿已保留。';
        }
        finally {
            if (key === this.sessionKey) {
    ...
    async refreshTarget() {
        ...
        if (!primary || (replyTarget && !replyTarget.valid && replyTarget.id === primary)) {
            await this.refresh();
            return;
        }
        const thread = this.state.threads.get(primary) ?? freshThread();
        this.state.threads.set(primary, thread);
    ...
    async loadThread(commentId, more = false) {
        ...
        if (!thread) {
            thread = freshThread();
            this.state.threads.set(commentId, thread);
        }
        this.state.expanded.add(commentId);
        if (thread.loaded && !more && !thread.stale) {
            this.onChange?.();
            return;
        }
        await this.fetchPage(this.state, thread, this.episodeId, this.state.order, commentId, thread.stale);
    }
    async fetchPage(episode, target, episodeId, order, commentId, refresh = false) {
        if (!this.connected || !episodeId || (!refresh && (target.loading || (target.loaded && (target.complete || !target.cursor)))))
            return;
        ...
        const notify = () => { if (this.state === episode && this.episodeId === episodeId && (commentId || episode.order === order))
            this.onChange?.(); };
        ...
        notify();
        try {
            const page = await this.call(commentId ? 'comments.thread' : 'comments.list', { epoch, episodeId, ...(commentId ? { commentId } : { order }), cursor });
            if (!current())
                return;
            ...
            if (!commentId)
                for (const item of page.items)
                    episode.confirmed.delete(item.id);
            else
                for (const item of page.items)
                    episode.threads.get(commentId)?.confirmed.delete(item.id);
            for (const item of page.items) {
                const replyTarget = episode.composers.get(item.id)?.replyTarget;
                if (replyTarget && (commentId ? item.primaryCommentId === replyTarget.primaryCommentId : item.id === replyTarget.primaryCommentId)) {
                    replyTarget.valid = true;
                    replyTarget.rejected = false;
                }
            }
            if (cursor)
...
export function renderComments(controller, login) {
    ...
    if (state.replyTarget) {
        ...
        const cancel = button('取消回复', () => { if (controller.cancelReply())
            document.querySelector('.comment-draft')?.focus(); }, 'text-button');
    ...
    const send = el('button', 'button primary', state.sending ? '正在发表…' : state.replyTarget ? '发送回复' : '发表评论');
    send.type = 'submit';
    ...
    if (state.sent) {
        const sent = el('p', 'comment-sent', state.replyTarget ? '回复已发表' : '评论已发表');
    ...
    const sorts = el('div', 'comment-order-tabs');
    sorts.setAttribute('role', 'tablist');
    sorts.setAttribute('aria-label', '评论排序');
    for (const [order, label] of [['hot', '热门'], ['latest', '最新']]) {
        const tab = button(label, () => controller.selectOrder(order), 'comment-order-tab');
        tab.id = `comment-order-${order}`;
        tab.setAttribute('role', 'tab');
        tab.setAttribute('aria-selected', String(state.order === order));
        tab.setAttribute('aria-controls', 'comment-order-list');
        tab.tabIndex = state.order === order ? 0 : -1;
    ...
    if (state.confirmed.size) {
        const published = el('section', 'comment-published');
        published.setAttribute('aria-label', '刚刚发表');
        published.append(el('h3', '', '刚刚发表'), el('p', 'muted', '已收到发表确认；列表中的位置以平台排序为准。'));
        for (const item of [...state.confirmed.values()].reverse()) {
            const entry = el('article', 'comment');
            entry.dataset.commentId = item.id;
            entry.append(el('strong', '', item.author.nickname), el('p', 'comment-text', item.text));
            published.append(entry);
        }
        root.append(published);
    }
    const drawPage = (target, container, load, reply = false, primaryCommentId) => {
        for (const item of target.items) {
            ...
            const date = el('time', 'muted');
            date.dateTime = item.createdAt;
            const stamp = new Date(item.createdAt);
            date.textContent = Number.isNaN(stamp.getTime()) ? '' : stamp.toLocaleString('zh-CN');
            meta.append(date);
            card.append(meta, el('p', 'comment-text', item.text));
            if (reply && item.replyTo) {
                const reference = el('div', 'comment-reference');
                reference.append(el('span', '', `回复 @${item.replyTo.nickname || '听友'}`), el('p', '', item.replyTo.summary));
                card.append(reference);
            }
            const replyAction = button('回复', () => {
                if (controller.selectReply(item.id, reply ? primaryCommentId : undefined))
                    document.querySelector('.comment-draft')?.focus();
            }, 'text-button comment-reply-action');
            replyAction.disabled = state.sending;
            card.append(replyAction);
            if (!reply && (item.replyCount !== undefined || state.threads.has(item.id))) {
                ...
                const toggle = button(expanded ? `收起回复${count}` : `查看回复${count}`, () => {
                    if (expanded) {
                        controller.state.expanded.delete(item.id);
                        controller.onChange?.();
                    }
                    else {
                        void controller.loadThread(item.id);
                        controller.onChange?.();
                    }
                ...
                if (expanded) {
                    ...
                    replies.dataset.primaryCommentId = item.id;
                    if (thread?.confirmed.size) {
                        const feedback = el('section', 'comment-reply-published');
                        feedback.setAttribute('aria-label', '刚刚回复');
                        feedback.append(el('h3', '', '刚刚回复'));
                        for (const sent of thread.confirmed.values()) {
                            const entry = el('article', 'comment');
                            entry.dataset.commentId = sent.id;
                            entry.append(el('strong', '', sent.author.nickname), el('p', 'comment-text', sent.text));
                            if (sent.replyTo)
                                entry.append(el('p', 'comment-reference', `回复 @${sent.replyTo.nickname}：${sent.replyTo.summary}`));
                            feedback.append(entry);
                        }
                        replies.append(feedback);
                    }
                    if (thread)
                        drawPage(thread, replies, () => controller.loadThread(item.id, true), true, item.id);
                    card.append(replies);
                }
            }
        ...
        if (target.loading) {
            const status = el('p', 'muted', '正在加载评论…');
            status.setAttribute('role', 'status');
            container.append(status);
        }
        else if (target.cursor && !target.complete && !target.error)
    ...
    list.setAttribute('aria-labelledby', `comment-order-${state.order}`);
    list.dataset.order = state.order;
    drawPage(controller.state, list, () => controller.load());
    root.append(list);
    return root;
}
