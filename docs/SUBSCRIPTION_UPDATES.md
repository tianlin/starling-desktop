...
在左侧“订阅更新”集中浏览已订阅播客的新单集。支持刷新、逐页加载、播放、加入稍后听、查看单集或播客详情，以及返回原列表位置。列表按已加载单集的发布时间排列，按本机日期分组；无效日期放在“日期未知”。现有播放器、队列和账号功能继续共用。首版不包含星标、未听筛选、新集红点、通知或后台轮询。
...
开发基线为 `a4b6deb`，独立分支 `codex/subscription-updates`。原 `codex/comments` 后续提交没有自动合入本分支。
