...
候选 r6 已完整构建、漏洞扫描通过，20 项组件/63 份声明已重新生成。媒体测试已接入 CI，其 Linux 结果以对应提交的 Actions 为准。本地机器可读结果在 docs/test-results/media.json。此证据不代替 WebView2、任意格式文件、原生设备或 PRD 规定的 3 轮各 4 小时持续收听验收。
...
## 评论回复与楼中楼增量（2026-09-12）

一级和楼中楼回复复用单次发送通道；本地验证同会话、单集、已读根评论及直接对象的归属，结果关联不符按未确认处理。草稿、错误及不确定结果按目标保留；确认回复只进入所属线程，后台刷新及退出账号晚结果均不会跳页或串入一级列表。未知上游业务码保留明确拒绝状态，不猜测“被删除”或退化为一级发表。

最终工程检查：根 Go 模块 135 项测试/子测试通过，5 个显式启用的 live 测试在常规测试中跳过；根和 desktop 两模块 go test ./...、go vet ./... 均通过（desktop 无独立测试用例）。前端 56 项通过，其中评论 26 项；合成浏览器评论 16 组、既有应用 16 组、编码媒体 6 组、媒体错误 4 组全部通过。新增浏览器覆盖一级及楼中楼回复读回、每目标草稿、禁止回复、结果丢失后的线程刷新核对、发送中锁定、退出账号晚响应及阅读位置/键盘/900×700 布局。播放器保持同一音频元素连续播放，显式退出账号按原行为暂停。

独立只读源码审查未发现未处理的正确性、隐私或竞态问题。截图复核发现回复数仍显示旧根列表计数，新增浏览器断言先失败后修复：完整线程采用已读取及已确认 ID 去重数量，部分线程保留平台计数下限。

独立 Windows 候选已构建：`build/Starling-candidate-replies.exe`，11,405,824 bytes，SHA256 `6c0de5c136e4a99b3d70da04e5d962bd34da5d40a181497e3665a08ed4cb1da9`。构建使用稳定 Call 桥并跳过绑定辅助程序；没有启动候选、终止或替换正在使用的程序。候选构建通过不等于原生 WebView2 实机验收。

真实发表评论和回复仍未执行。用户需在候选界面主动选择目标、输入正文、点击发送，并核对平台可见性；自动化只向本机合成服务发表。引用的社区协议不代表直接核验最新版手机 App 的界面、默认项或热度算法。所有此次提交仅保留本地，未推送或核验新远端 CI。

最终只读关系复核：使用新回复解析器读取两个真实已有线程，均为预期 1 条、返回 1 条、complete=true，所属 primary 与直接 reply reference 各 1 条。HOT/TIME 各两页及时间方向再次通过。没有真实写请求或凭据续期/保存。

## 主干合并验证（2026-09-12）

将评论排序与楼中楼回复分支合入已包含订阅更新流的 main。导航清理保留评论阅读位置保存及更新流取消/返回状态；两组样式同时保留，重新生成内嵌前端，适配器版本标记合并后的评论与更新能力。根目录和 desktop 两个 Go 模块的测试与 vet 通过；前端 67 项通过；合成浏览器评论 16 组、订阅更新 7 组、既有应用 16 组通过。本次合并检查没有发送真实评论或回复。
