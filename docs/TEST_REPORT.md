...
## 探索与账号订阅增量（2026-09-12）

实现搜索＋轻量发现首页、三类平台搜索、账号隔离 SQLite 历史、用户资料及创作节目、共享订阅状态和单次写入流程。搜索与分页按关键词、分类、账号 epoch 和请求代次隔离；订阅不确定结果先读回，禁止自动重放。确认记录保护新订阅，完整刷新纳入后退出回退，缓存失败仍保留已确认状态。接口契约及实测边界见 [DISCOVERY_API.md](DISCOVERY_API.md)，使用说明见 [DISCOVERY.md](DISCOVERY.md)。

最终检查：根目录及 desktop 两个 Go 模块的 `go test ./...`、`go vet ./...` 均通过（desktop 无独立测试用例）；前端 **101 项通过**。合成浏览器共 **57 组通过**：探索 8、订阅更新 7、核心 16、评论 16、编码媒体 6、媒体错误 4。最终前端修复后再次运行探索、订阅更新及核心全部通过。探索验收走通用户作品→订阅→我的订阅，以及单集→播放→稍后听→详情返回；涵盖输入法、未连接继续搜索、分页失败、结果待确认、历史删除、无作品用户、窄窗口、键盘和同一音频元素连续播放。

独立源码复核发现并修复原始用户数限制、数据库失败丢失内存确认、普通 ON 状态读取误触更新失效、旧链接抢占新搜索、前端确认缓存不退出及空草稿翻页问题；后续针对性复核确认全部解决。最后三项增加 15 项测试，其中修复前有 14 项复现失败，修复后全部通过。

真实只读：科技查询得到 7 条推荐词，节目/单集/用户各 20+20 条，用户作品 0 档；有知有行查询得到节目 7 条、用户 12 条、单集 20+20 条，用户作品 5 档；两次订阅状态读取均成功。部分搜索缺少结束元数据，保留未知结束提示。只读探针未续期、保存凭据或发送真实订阅 POST。真实订阅及平台读回、原生 WebView2 交互仍须由用户在候选界面主动验收。

独立 Windows 候选：`build/Starling-candidate-explore.exe`，11,581,440 bytes，SHA256 `356e263458c5c457f27f9d45c086388aaf4a108f7bf00d6ae255460c167e5e1b`。实际二进制 `govulncheck v1.8.0` 扫描无命中。构建跳过绑定辅助程序；未启动候选、关闭或替换现有程序。源代码保留在 `codex/explore` 独立工作树，本次未推送或核验远端 CI。

## 搜索终页修正（2026-09-12）

真实完整分页确认节目、单集、用户搜索均以省略 loadMoreKey 和 hasMore 表示结束；只在搜索适配器应用该规则，通用分页、矛盾标志及异常响应校验保持原样。新增 18 个回归场景先失败后通过，覆盖三类搜索的 0/1/20 条终页及首屏/续页。前端 101 项、两个 Go 模块测试与 vet、探索浏览器 8 组通过，浏览器新增结束文案断言。修复后真实只读搜索脑放电波：节目 1 条 complete=true，用户 0 条 complete=true，单集仍保留有效翻页；未进行账号写操作。
